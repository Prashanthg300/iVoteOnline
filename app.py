...
def viewresults():
    id = request.args['id']
    sql8 = """
        SELECT r.fname, r.lname, c.symbol, c.admno, count(*) 
        FROM electoralroll e, candidates c, registration r 
        WHERE e.cid='{}' 
            AND e.candidateid IN (SELECT candidateid FROM candidates c WHERE c.cid='{}') 
            AND c.candidateid = e.candidateid 
            AND c.admno = r.admno 
        GROUP BY e.candidateid, r.fname, r.lname
    """.format(id, id)
    data = select_records(sql8)	
...
def detectfaceindex():
    ...
    if faceCascade.empty():
        ...
        return "Error: Unable to load cascade classifier."
    ...
    if not videoCap.isOpened():
        ...
        return "Error: Unable to open video capture."
    ...
    while True:
        ...
        print("Face recognition result:", data[1])
